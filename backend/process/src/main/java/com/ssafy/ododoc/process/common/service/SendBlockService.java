...
public class SendBlockService {
    ...
    public void sendRequest(LinkedHashMap<String, FileBlockDto> content, MessageRecord messageRecord) {
        log.info("sendRequest 호출!!!");
        WebClient webClient = WebClient.builder()
                .baseUrl(URI)
                .defaultHeader("Content-type", "application/json")
                .defaultHeader("Authorization", messageRecord.getAccessToken())
                .build();

        String type = "";

        switch(messageRecord.getDataType()) {
            case OUTPUT -> type = "success";
            case ERROR -> type = "fail";
            case KEYWORD -> type = "keyword";
            case SEARCH -> type = "search";
        ...
        LinkedHashMap<String, Object> requestBody = new LinkedHashMap<>();
        requestBody.put("connectedFileId", messageRecord.getConnectedFileId());
        requestBody.put("type", type);
        requestBody.put("fileBlock", content);
