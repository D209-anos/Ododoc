...
public class DirectoryApiTest extends ApiTest {
    ...
    void 디렉토리_폴더생성_성공_상위없음_200() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andDo(document(DEFAULT_RESTDOC_PATH, "디렉토리(폴더/파일)를 생성하는 API 입니다." +
                        ...
                        "<br> - 상위 폴더 아이디가 있다면, 1 이상이어야 합니다. 1 미만인 경우, <b>400 Bad Request</b>가 반환됩니다." +
    ...
    void 디렉토리_삭제_성공_200() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andDo(document(DEFAULT_RESTDOC_PATH, "폴더/파일을 삭제하는 API 입니다." +
                        ...
                        "<br> - directoryId에 해당하는 폴더/파일이 이미 삭제(휴지통, 영구삭제) 되었다면, <b>410 Gone</b>이 반환됩니다.",
    ...
    @Test
    void 디렉토리_삭제_삭제된디렉토리_410() throws Exception {
        String token = memberTestUtil.회원가입_토큰반환(mockMvc);
        Long directroyId = directoryTestUtil.폴더_생성(token, mockMvc);

        directoryTestUtil.폴더_삭제(token, directroyId, mockMvc);

        mockMvc.perform(
                delete("/directory/{option}/{directoryId}", "trashbin", directroyId)
                        .header(AUTH_HEADER, token)
        )
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value(410))
    ...
    void 디렉토리명_변경_성공_200() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andDo(document(DEFAULT_RESTDOC_PATH, "디렉토리명을 변경하는 API 입니다." +
                                ...
                                "<br> - id에 해당하는 폴더/파일이 이미 삭제(휴지통, 영구삭제) 되었다면, <b>410 Gone</b>이 반환됩니다.",
    ...
    @Test
    void 디렉토리명_변경_삭제된디렉토리_410() throws Exception {
        String token = memberTestUtil.회원가입_토큰반환(mockMvc);
        Long directoryId = directoryTestUtil.폴더_생성(token, mockMvc);
